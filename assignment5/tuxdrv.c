...
static int my_major = MYCDRV_MAJOR, my_minor = 0, my_nr_devs = MYCDRV_NR_DEVS, my_quantum = MYCDRV_QUANTUM, my_qset = MYCDRV_QSET;

module_param(my_major, int, S_IRUGO);
module_param(my_minor, int, S_IRUGO);
module_param(my_nr_devs, int, S_IRUGO);
module_param(my_quantum, int, S_IRUGO);
module_param(my_qset, int, S_IRUGO);
...
static int __init my_init(void)
{
	...
	pr_info("\nSucceeded in registering character device %s hugh pwned this\n", MYDEV_NAME);
	return 0;
}
...
static void __exit my_exit(void)
{
	...
	pr_info("\ndevice unregistered hugh pwned this\n");
	kfree(ramdisk);
}
